...
> Ekkon ist **fester Bestandteil des Cores** (`app/Ekkon/`), kein Paket mehr. Bis 2026-09 war
> es `do1emu/module-ekkon` mit dem Namensraum `Intranet\Modules\Ekkon`; ältere Fachmodule
> benutzen diese Namen weiter – `App\Ekkon\Altnamen` bildet sie auf `App\Ekkon` ab. Neue
> Module schreiben gleich `App\Ekkon\…`. Pfadangaben `src/…` in diesem Text meinen bei Ekkon
> selbst `app/Ekkon/…`, bei einem Fachmodul dessen eigenes `src/`.
